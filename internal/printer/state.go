...
const (
	// nozzleTempDeviationThreshold is the max delta between actual and target nozzle
	// temperature before emitting a NozzleTempAnomaly event.
...
type stateTracker struct {
	...
	prevAMSTray string
	prevHMS    map[string]bool // dedup HMS alerts by message
	anomalyEmitted bool       // avoid spamming temp anomaly events
}

// newStateTracker creates a tracker for the given printer.
...
func (st *stateTracker) update(r *parsedReport) []event.Event {
	...
	if r.GcodeState != st.prevState && r.GcodeState != event.StateUnknown {
		if evt, ok := st.transitionEvent(st.prevState, r.GcodeState, base); ok {
			events = append(events, evt)
		}
		st.prevState = r.GcodeState

		// Reset anomaly flag on state change
	...
	// Detect new HMS alerts (deduplicated).
	...
	if r.AMSTrayNow != "" && r.AMSTrayNow != st.prevAMSTray && st.prevAMSTray != "" {
		e := base
	...
	if r.GcodeState == event.StateRunning && r.Thermals.NozzleTarget > 0 {
		delta := math.Abs(r.Thermals.NozzleActual - r.Thermals.NozzleTarget)
		if delta > nozzleTempDeviationThreshold && !st.anomalyEmitted {
...
// transitionEvent maps a state change to an event type.
func (st *stateTracker) transitionEvent(from, to event.GcodeState, base event.Event) (event.Event, bool) {
	e := base
	...
	case event.StatePrepare:
		// PREPARE is a pre-print state; emit started when transitioning from idle/unknown
		if from == event.StateIdle || from == event.StateUnknown || from == event.StateFinish {
			e.Type = event.PrintStarted
		} else {
			return event.Event{}, false
		}
	case event.StateIdle:
		// IDLE after a print may not warrant an event (already handled by FINISH/FAILED)
		return event.Event{}, false
	default:
		return event.Event{}, false
	}
...
// reset clears tracked state. Used on reconnect.
